...
@dp.table(
    name="fineguard.gold.fraud_card_alert",
    comment="High value transaction alert"
)
def farud_card_alert():
    transactions=spark.readStream.table("fineguard.silver.transactions")
    fraud_watchlist=saprk.readStream.table("fineguard.silver.fraud_watchlist)

    customers=spark.readStream.table("fineguard.silver.customers)
    ...
    fraud_detection=(transactions_with_watermark.join(
